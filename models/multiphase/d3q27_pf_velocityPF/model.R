...
if (Options$q27){
    ...
    if (Options$PFMRT || Options$PFSRT){ 
	M_h = M
	invM_h = invM
     }
} else {
	...
	m0_h = matrix(1,1,15)
	m1_h = U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3] - 2
    m2_h = 0.5 * (15 * (U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3]) * (U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3]) - 55 * (U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3]) + 32)
    m3_h = U_h[,1]
    m4_h = 0.5 * (5 * (U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3]) -13) * U_h[,1]
    m5_h = U_h[,2]
    m6_h = 0.5 * (5 * (U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3]) -13) * U_h[,2]
    m7_h = U_h[,3]
    m8_h = 0.5 * (5 * (U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3]) -13) * U_h[,3]
    m9_h = 3 * U_h[,1] * U_h[,1] - (U_h[,1] * U_h[,1] + U_h[,2] * U_h[,2] + U_h[,3] * U_h[,3])
    m10_h = U_h[,2] * U_h[,2] - U_h[,3] * U_h[,3]
    m11_h = U_h[,1] * U_h[,2]
    m12_h = U_h[,2] * U_h[,3]
    m13_h = U_h[,1] * U_h[,3]
    m14_h = U_h[,1] * U_h[,2] * U_h[,3]

    M_h = rbind(m0_h,m1_h,m2_h,m3_h,m4_h,m5_h,m6_h,m7_h,m8_h,m9_h,m10_h,m11_h,m12_h,m13_h,m14_h)
    m_h = PV(c(paste("m_h[",1:PF_velocities-1,"]",sep="")))
    invM_h = solve(M_h)
    }
}
omega = PV("omega_phi")
